...
function hmm_independent_lik(data, results::T; subject=0, params=nothing, delay_turn_bias=false, rewscaled=false, add_leaf=true, record=false) where T <: EMResultsAbstract
    ...
    hmm_independent_lik(data; delay_turn_bias, rewscaled, add_leaf, record, d...)
end
...
function find_Q_vals_hmm_independent(df, results; add_leaf=true, rewscaled, delay_turn_bias, params=nothing, subjlevel=:daynum)
    data = copy(df)
    data[:, :sub] = data[:, subjlevel]
...
run_hmm_independent_stay_depletion(data; kwargs...) = run_hmm_independent(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_independent_turn_depletion(data; kwargs...) = run_hmm_independent(data; add_leaf=false, add_βleaf=false, add_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_independent_spatial_depletion(data; kwargs...) = run_hmm_independent(data; add_leaf=false, add_βleaf=false, add_spatial_bias=true, add_depletion_factor=true, kwargs...)

run_hmm_independent_stay_γ2_depletion(data; kwargs...) = run_hmm_independent(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_independent_stay_retainbelief_depletion(data; kwargs...) = run_hmm_independent(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_retain_belief=true, add_depletion_factor=true, kwargs...)
