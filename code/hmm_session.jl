...
function hmm_session_lik_stem_inner!(Qstem, Q, prevs, prevl, βgo, βstay, stay_bias, γ2)
    Qstem .= βgo .* mean.(Q)
...
function hmm_session_lik_turn_inner!(Qstem, prevs, turn_bias, spatial_bias)
    Qstem[mod1(prevs + 1, 3)] += turn_bias
    Qstem[mod1(prevs + 1, 3)] += spatial_bias[prevs]
end

function hmm_session_lik_leaf_inner!(Qleaf, Q, stemchoice, βleaf, leaf_turn_bias, leaf_spatial_bias)
    # Leaf choice
    Qleaf .= Q[stemchoice]
    # Add turn bias
...
If 'record':
    ...
    reward_entropy
"""
function hmm_session_lik(df, ϕ::Array{Float64, 2}, volatility, βgo::U, βstay, βleaf, stay_bias, turn_bias, spatial_bias, leaf_turn_bias, leaf_spatial_bias, γ2, depletion_factor, retain_belief, delay_turn_bias::Bool, rewscaled::Bool, add_leaf::Bool, record::Bool) where U
    nstates = size(ϕ, 1)
    ntrials = length(df)
    ...
    # (dates, sessions, leaf, leafchoice, stemchoice, reward) = hmm_session_lik_extract_df(df)
    dates = df.date
    sessions = df.session
    leaf = df.leaf
    ...
    for date in unique(dates)
        ...
        for session in unique(d_sessions)
            ...
            @inbounds for t in t_ind
                ...
                Q[3][1] = Qtemp[5]
                Q[3][2] = Qtemp[6]
                hmm_session_lik_stem_inner!(Qstem, Q, prevs, prevl, βgo, βstay, stay_bias, γ2)
                if (prevs > 0)
                    ...
                    if delay_turn_bias  # Whether we add on the turn bias before or after we calculate stay/go
                        ...
                        hmm_session_lik_turn_inner!(Qstem, prevs, turn_bias, spatial_bias)
                    else
                        hmm_session_lik_turn_inner!(Qstem, prevs, turn_bias, spatial_bias)
                        lp_stay = Qstem[prevs] - logsumexp(view(Qstem, :))
                        lp_go = logsumexp(view(Qstem, [mod1(prevs + 1, 3), mod1(prevs + 2, 3)])) - logsumexp(view(Qstem, :))
                ...
                if (add_leaf && (prevs != stemchoice[t]))  # If stem switch, add leaf bias
                    hmm_session_lik_leaf_inner!(Qleaf, Q, stemchoice[t], βleaf, leaf_turn_bias, leaf_spatial_bias)
...
function hmm_session_lik(df, ϕ; volatility=1.0/30.0, βgo=0., βstay=0., βleaf=0., stay_bias=0., turn_bias=0., spatial_bias=[0., 0, 0], leaf_turn_bias=0., leaf_spatial_bias=[0., 0, 0], γ2=0., depletion_factor=1., retain_belief=0., delay_turn_bias=false, rewscaled=false, add_leaf=true, record=false)
    hmm_session_lik(df, ϕ, volatility, βgo, βstay, βleaf, stay_bias, turn_bias, spatial_bias, leaf_turn_bias, leaf_spatial_bias, γ2, depletion_factor, retain_belief, delay_turn_bias, rewscaled, add_leaf, record)
end
...
"""
function hmm_session_lik(data, ϕ, results::T; subject=0, params=nothing, delay_turn_bias=false, rewscaled=false, add_leaf=true, record=false) where T <: EMResultsAbstract
    d = Dict{Symbol, Any}()
    ...
    hmm_session_lik(data, ϕ; delay_turn_bias=delay_turn_bias, rewscaled=rewscaled, add_leaf=add_leaf, record=record, d...)
end
...
"""
function run_hmm_session(df; maxiter=100, emtol=1e-3, full=true, extended=false, quiet=false,
    ϕ=nothing,
    add_βleaf=false,
    add_stay_bias=false,
    add_turn_bias=false,
    ...
    function fn(params, data)
        ...
        return hmm_session_lik(data, ϕ, volatility, βgo, βstay, βleaf, stay_bias, turn_bias, spatial_bias, leaf_turn_bias, leaf_spatial_bias, γ2, depletion_factor, retain_belief, delay_turn_bias, rewscaled, add_leaf, false)
    end
...
#     dfs = []
#     for i in 1:ndays
#         (liks[i], df) = hmm_session_lik(view(data, data.daynum .== i, :), get_contingencies(), results;
...
# Non-Depletion
run_hmm_session_leaf(data; kwargs...) = run_hmm_session(data; add_βleaf=true, kwargs...)
run_hmm_session_leaf_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_γ2=true, kwargs...)
run_hmm_session_leaf_retainbelief(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_retain_belief=true, kwargs...)
run_hmm_session_leaf_stay(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, kwargs...)
run_hmm_session_leaf_turn(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_turn_bias=true, kwargs...)
run_hmm_session_leaf_spatial(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_spatial_bias=true, kwargs...)
run_hmm_session_leaf_leafspatial(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_leaf_spatial_bias=true, kwargs...)
run_hmm_session_leaf_leafturn(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_leaf_turn_bias=true, kwargs...)

run_hmm_session_leaf_stay_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_γ2=true, kwargs...)
run_hmm_session_leaf_stay_retainbelief(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_retain_belief=true, kwargs...)
run_hmm_session_leaf_stay_turn(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, kwargs...)
run_hmm_session_leaf_stay_spatial(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, kwargs...)
run_hmm_session_leaf_stay_leafspatial(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_leaf_spatial_bias=true, kwargs...)
run_hmm_session_leaf_stay_leafturn(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_leaf_turn_bias=true, kwargs...)
run_hmm_session_leaf_stay_turn_leafspatial(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_leaf_spatial_bias=true, kwargs...)
run_hmm_session_leaf_stay_turn_leafturn(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_leaf_turn_bias=true, kwargs...)
run_hmm_session_leaf_stay_spatial_leafspatial(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_leaf_spatial_bias=true, kwargs...)
run_hmm_session_leaf_stay_spatial_leafturn(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_leaf_turn_bias=true, kwargs...)

run_hmm_session_leaf_turn_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_turn_bias=true, kwargs...)
run_hmm_session_leaf_spatial_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_spatial_bias=true, kwargs...)
run_hmm_session_leaf_turn_leafspatial_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_turn_bias=true, add_leaf_spatial_bias=true, kwargs...)
run_hmm_session_leaf_turn_leafturn_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_turn_bias=true, add_leaf_turn_bias=true, kwargs...)
run_hmm_session_leaf_spatial_leafspatial_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_spatial_bias=true, add_leaf_spatial_bias=true, kwargs...)
run_hmm_session_leaf_spatial_leafturn_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_spatial_bias=true, add_leaf_turn_bias=true, kwargs...)

run_hmm_session_leaf_stay_turn_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_γ2=true, kwargs...)
run_hmm_session_leaf_stay_spatial_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_γ2=true, kwargs...)
run_hmm_session_leaf_stay_leafturn_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_leaf_turn_bias=true, add_γ2=true, kwargs...)
run_hmm_session_leaf_stay_leafspatial_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_leaf_spatial_bias=true, add_γ2=true, kwargs...)
run_hmm_session_leaf_stay_turn_leafspatial_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_leaf_spatial_bias=true, add_γ2=true, kwargs...)
run_hmm_session_leaf_stay_turn_leafturn_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_leaf_turn_bias=true, add_γ2=true, kwargs...)
run_hmm_session_leaf_stay_spatial_leafspatial_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_leaf_spatial_bias=true, add_γ2=true, kwargs...)
run_hmm_session_leaf_stay_spatial_leafturn_γ2(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_leaf_turn_bias=true, add_γ2=true, kwargs...)

# Depletion
run_hmm_session_leaf_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_retainbelief_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_retain_belief=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_turn_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_spatial_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_leafspatial_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_leaf_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_leafturn_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_leaf_turn_bias=true, add_depletion_factor=true, kwargs...)

run_hmm_session_leaf_stay_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_retainbelief_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_retain_belief=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_turn_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_spatial_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_leafspatial_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_leaf_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_leafturn_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_leaf_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_turn_leafspatial_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_leaf_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_turn_leafturn_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_leaf_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_spatial_leafspatial_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_leaf_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_spatial_leafturn_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_leaf_turn_bias=true, add_depletion_factor=true, kwargs...)

run_hmm_session_leaf_turn_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_spatial_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_turn_leafspatial_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_turn_bias=true, add_leaf_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_turn_leafturn_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_turn_bias=true, add_leaf_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_spatial_leafspatial_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_spatial_bias=true, add_leaf_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_spatial_leafturn_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_spatial_bias=true, add_leaf_turn_bias=true, add_depletion_factor=true, kwargs...)

run_hmm_session_leaf_stay_turn_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_spatial_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_leafturn_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_leaf_turn_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_leafspatial_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_leaf_spatial_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_turn_leafspatial_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_leaf_spatial_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_turn_leafturn_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_turn_bias=true, add_leaf_turn_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_spatial_leafspatial_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_leaf_spatial_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_leaf_stay_spatial_leafturn_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_βleaf=true, add_stay_bias=true, add_spatial_bias=true, add_leaf_turn_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)

# Depletion without leaf, might make depletion / non-depletion comparison better?
run_hmm_session_base(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_γ2=false, add_depletion_factor=false, kwargs...)
run_hmm_session_γ2(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_γ2=true, add_depletion_factor=false, kwargs...)
run_hmm_session_retainbelief(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_retain_belief=true, add_depletion_factor=false, kwargs...)
run_hmm_session_stay(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_depletion_factor=false, kwargs...)
run_hmm_session_turn(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_turn_bias=true, add_depletion_factor=false, kwargs...)
run_hmm_session_spatial(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_spatial_bias=true, add_depletion_factor=false, kwargs...)

run_hmm_session_stay_γ2(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_γ2=true, add_depletion_factor=false, kwargs...)
run_hmm_session_stay_retainbelief(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_retain_belief=true, add_depletion_factor=false, kwargs...)
run_hmm_session_stay_turn(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_turn_bias=true, add_depletion_factor=false, kwargs...)
run_hmm_session_stay_spatial(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_spatial_bias=true, add_depletion_factor=false, kwargs...)

run_hmm_session_turn_γ2(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_turn_bias=true, add_depletion_factor=false, kwargs...)
run_hmm_session_spatial_γ2(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_spatial_bias=true, add_depletion_factor=false, kwargs...)
run_hmm_session_stay_turn_γ2(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_turn_bias=true, add_γ2=true, add_depletion_factor=false, kwargs...)
run_hmm_session_stay_spatial_γ2(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_spatial_bias=true, add_γ2=true, add_depletion_factor=false, kwargs...)

run_hmm_session_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_depletion_factor=true, kwargs...)
run_hmm_session_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_retainbelief_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_retain_belief=true, add_depletion_factor=true, kwargs...)
run_hmm_session_stay_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_turn_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_spatial_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_spatial_bias=true, add_depletion_factor=true, kwargs...)

run_hmm_session_stay_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_stay_retainbelief_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_retain_belief=true, add_depletion_factor=true, kwargs...)
run_hmm_session_stay_turn_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_stay_spatial_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_spatial_bias=true, add_depletion_factor=true, kwargs...)

run_hmm_session_turn_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_turn_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_spatial_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_spatial_bias=true, add_depletion_factor=true, kwargs...)
run_hmm_session_stay_turn_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_turn_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)
run_hmm_session_stay_spatial_γ2_depletion(data; kwargs...) = run_hmm_session(data; add_leaf=false, add_βleaf=false, add_stay_bias=true, add_spatial_bias=true, add_γ2=true, add_depletion_factor=true, kwargs...)