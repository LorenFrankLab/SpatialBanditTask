...
"""
So far we've computed the MAP parameters, and can compute a point estimate P(Y | M, x)
but really want to approximate the likelihood of the model more globally, P(Y | M).

The approximation we make is that P(Y | M) is normally distributed around x_MAP

See: http://www2.stat.duke.edu/~st118/sta250/laplace.pdf
"""
function heldoutsubject_laplace(mu, sigma, data, likfun; startx = mu)
	nparam = length(mu)

	# lik is negative here (since it's directly the result from optimize)
