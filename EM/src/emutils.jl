...
# MAP estimate for x given a prior
# log(theta | x) = log(p(x | theta)) + log(p(theta))
# log(p(x | theta)) : likelihood
# log(p(theta)) : prior
function gaussianprior(params,mu,sigma,data,likfun)
	d = length(params)

	# Gaussian penalizing parameter distance from group-level mu
...
# Return a flattened array of betas, followed by either upper triangle sigma or diagonal sigma
