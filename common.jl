...
function optimizesubject(likfun, startx)
    ...
    #lower = [0.,0.,0.,-Inf,-Inf,-Inf,-Inf,-Inf,-Inf,-Inf,-Inf]
    #upper = [Inf, Inf, Inf, Inf,Inf,Inf,Inf,Inf,Inf,Inf,Inf]
    #suboptimizer = BFGS(linesearch=Optim.LineSearches.BackTracking())
    #a = optimize(likfun, lower, upper,startx,Fminbox(suboptimizer))
...
function gaussianprior(params,mu,sigma,data,likfun)
	...
	return (nll - lp[1]) # it is -(loglikelihood+lp). Due to the negative sign within likfun, hence the negaive here before lp[1].
end
