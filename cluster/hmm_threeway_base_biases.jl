...
function find_Q_vals_by_day(data, results; add_leaf=true, rewscaled, delay_turn_bias)
    ...
    dfs = []
    for i in 1:ndays
        (liks[i], df) = hmm_lik(view(data, data.daynum .== i, :), get_contingencies_threeway_base(), results;
...
base_dir = "../results/hmm_threeway_base_biases"
i = parse(Int, ARGS[1])
...
function run_fn(fn_name, fn, fn_add_leaf, rewscaled, delay_turn_bias, flag_loocv, full)
	...
	results_loocv = fn(data; ϕ=get_contingencies_threeway_base(), extended=true, rewscaled=rewscaled, delay_turn_bias=delay_turn_bias, loocv_data=results, full)
        save("$(base_dir)_loocv/$(fname_loocv).jld2", "results_loocv", results_loocv; compress=true)
    else
        results = fn(data; ϕ=get_contingencies_threeway_base(), extended=true, rewscaled=rewscaled, delay_turn_bias=delay_turn_bias, full)
