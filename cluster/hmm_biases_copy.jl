...
function hmm_biases_copy(ARGS)
    println(ARGS)
    println("going to start runnin gthe hmm biases copy fxn")
    ...
    save("../results/hmm_biases/$(fname)_rewscaled_delayturnbias_$(animal).jld2", "$(fname)_rewscaled_delayturnbias_$(animal)", results; compress=true)
end

function hmm_biases_sg(model_name, animal_name)
    #println(ARGS)
    println("going to start running the hmm biases spyglass fxn")
...
function hmm_biases_csv(RERUN, SAVE_Q, fn;
        filepath="..", animals=[
            "senor",
            "chimi",
            "j16",
...
function hmm_biases_dj(
    ...
    if RERUN
        if rewscaled==false
            ...
            save("$(behavior_model_fit_path)/$(fname)_$(animal).jld2", "$(fname)_$(animal)", results; compress=compress)
        else
            results = fn(data; maxiter=maxiter, extended=extended, rewscaled=rewscaled)
            save("$(behavior_model_fit_path)/$(fname)_rewscaled_$(animal).jld2", "$(fname)_rewscaled_$(animal)", results; compress=compress)
        end
    else
        results = load("$(behavior_model_fit_path)/hmm_$(fn)_$(animal).jld2", "hmm_$(fn)_$(animal)")
    end
    
    Qs = find_Q_vals_by_day_dj(animal, results, rewscaled=rewscaled)
    
    if SAVE_Q
        CSV.write("$(behavior_model_csv_path)/Q_vals_$(fn)_$(animal).csv", Qs)
    end
end
