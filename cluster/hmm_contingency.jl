...
if fn_ind == 1
	...
	save("../results/hmm_contingency/contingencies_base_$(animal).jld2", "contingencies_base_$(animal)", results)
elseif fn_ind == 2
	results = run_hmm_leaf_stay_turn_leafspatial(data; ϕ=get_contingencies_observed());
	save("../results/hmm_contingency/contingencies_observed_$(animal).jld2", "contingencies_observed_$(animal)", results)
else
	n = fn_ind - 2
	results = run_hmm_leaf_stay_turn_leafspatial(data; ϕ=get_contingencies(;n=n));
	save("../results/hmm_contingency/contingencies_$(n)_$(animal).jld2", "contingencies_$(n)_$(animal)", results)
end
