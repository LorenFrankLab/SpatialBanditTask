...
base_dir = "../results/hmm_nononlocal_biases"
subjlevel = :daynum
i = parse(Int, ARGS[1])
...
function run_fn(fn_name, fn, fn_add_leaf, rewscaled, delay_turn_bias, flag_loocv, full)
    ...
    if flag_loocv
        ...
        results_loocv = fn(data; extended=true, rewscaled=rewscaled, delay_turn_bias=delay_turn_bias, subjlevel, loocv_data=results, full, add_volatility, add_βgo, add_βstay)
        save("$(base_dir)_loocv/$(fname_loocv).jld2", "results_loocv", results_loocv; compress=true)
    else
        results = fn(data; extended=true, rewscaled=rewscaled, delay_turn_bias=delay_turn_bias, subjlevel, full, add_volatility, add_βgo, add_βstay)
